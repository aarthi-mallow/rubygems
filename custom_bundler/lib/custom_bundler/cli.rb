...
module CustomBundler
  class CLI < Thor
    ...
    def self.start(*)
      ...
      CustomBundler::SharedHelpers.print_major_deprecations!
    end
    ...
    def initialize(*args)
      ...
      custom_gemfile = options[:gemfile] || CustomBundler.settings[:gemfile]
      if custom_gemfile && !custom_gemfile.empty?
        CustomBundler::SharedHelpers.set_env "BUNDLE_GEMFILE", File.expand_path(custom_gemfile)
        CustomBundler.reset_settings_and_root!
      end

      CustomBundler.auto_switch

      CustomBundler.settings.set_command_option_if_given :retry, options[:retry]

      current_cmd = args.last[:current_command].name
      CustomBundler.auto_install if AUTO_INSTALL_CMDS.include?(current_cmd)
    rescue UnknownArgumentError => e
      raise InvalidOption, e.message
    ensure
      self.options ||= {}
      unprinted_warnings = CustomBundler.ui.unprinted_warnings
      CustomBundler.ui = UI::Shell.new(options)
      CustomBundler.ui.level = "debug" if options["verbose"]
      unprinted_warnings.each {|w| CustomBundler.ui.warn(w) }
    end
    ...
    def cli_help
      ...
      shell.say "CustomBundler commands:\n\n"
    ...
    default_task(CustomBundler.feature_flag.default_cli_command)
    ...
    def help(cli = nil)
      ...
      if man_pages.include?(command)
        ...
        if CustomBundler.which("man") && !man_path.match?(%r{^file:/.+!/META-INF/jruby.home/.+})
          Kernel.exec("man", man_page)
        else
          puts File.read("#{man_path}/#{File.basename(man_page)}.ronn")
        end
      elsif command_path = CustomBundler.which("bundler-#{cli}")
    ...
    def self.handle_no_command_error(command, has_namespace = $thor_runner)
      if CustomBundler.feature_flag.plugins? && CustomBundler::Plugin.command?(command)
        return CustomBundler::Plugin.exec_command(command, ARGV[1..-1])
      end

      return super unless command_path = CustomBundler.which("bundler-#{command}")
    ...
    long_desc <<-D
      ...
      all gems are found, CustomBundler prints a success message and exits with a status of 0.
      If not, the first missing gem is listed and CustomBundler exits status 1.
    D
    method_option "dry-run", type: :boolean, default: false, banner: "Lock the Gemfile"
    method_option "gemfile", type: :string, banner: "Use the specified gemfile instead of Gemfile"
    method_option "path", type: :string, banner: "Specify a different path than the system default ($BUNDLE_PATH or $GEM_HOME).#{" CustomBundler will remember this value for future installs on this machine" unless CustomBundler.feature_flag.forget_cli_options?}"
    def check
    ...
    long_desc <<-D
      Removes the given gems from the Gemfile while ensuring that the resulting Gemfile is still valid. If the gem is not found, CustomBundler prints a error message and if gem could not be removed due to any reason CustomBundler will display a warning.
    D
    ...
    method_option "standalone", type: :array, lazy_default: [], banner: "Make a bundle that can work without the CustomBundler runtime"
    ...
    method_option "trust-policy", alias: "P", type: :string, banner:       "Gem trust policy (like gem install -P). Must be one of " +
                                                                           CustomBundler.rubygems.security_policy_keys.join("|")
    ...
    def install
      ...
      require_relative "cli/install"
      CustomBundler.settings.temporary(no_install: false) do
    ...
    method_option "shebang", type: :string, banner: "Specify a different shebang executable name than the default (usually 'ruby')"
    method_option "standalone", type: :boolean, banner: "Make binstubs that can work without the CustomBundler runtime"
    ...
    long_desc <<-D
      ...
      are up to date, CustomBundler will exit with a status of 0. Otherwise, it will exit 1.
    ...
    method_option "all",  type: :boolean,
                          default: CustomBundler.feature_flag.cache_all?,
    ...
    unless CustomBundler.feature_flag.bundler_3_mode?
    ...
    desc "version", "Prints CustomBundler version information"
    def version
      ...
      if !cli_help && CustomBundler.feature_flag.print_only_version_number?
        CustomBundler.ui.info "#{CustomBundler::VERSION}#{build_info}"
      else
        CustomBundler.ui.info "CustomBundler version #{CustomBundler::VERSION}#{build_info}"
      end
    end
    ...
    def licenses
      CustomBundler.load.specs.sort_by {|s| s.license.to_s }.reverse_each do |s|
        gem_name = s.name
        license  = s.license || s.licenses

        if license.empty?
          CustomBundler.ui.warn "#{gem_name}: Unknown"
        else
          CustomBundler.ui.info "#{gem_name}: #{license}"
        end
      end
    end
    ...
    method_option :test, type: :string, lazy_default: CustomBundler.settings["gem.test"] || "", aliases: "-t", banner: "Use the specified test framework for your library",
                         enum: %w[rspec minitest test-unit],
                         desc: "Generate a test directory for your library, either rspec, minitest or test-unit. Set a default with `bundle config set --global gem.test (rspec|minitest|test-unit)`."
    method_option :ci, type: :string, lazy_default: CustomBundler.settings["gem.ci"] || "",
                       enum: %w[github gitlab circle],
                       desc: "Generate CI configuration, either GitHub Actions, GitLab CI or CircleCI. Set a default with `bundle config set --global gem.ci (github|gitlab|circle)`"
    method_option :linter, type: :string, lazy_default: CustomBundler.settings["gem.linter"] || "",
                           enum: %w[rubocop standard],
                           desc: "Add a linter and code formatter, either RuboCop or Standard. Set a default with `bundle config set --global gem.linter (rubocop|standard)`"
    method_option :github_username, type: :string, default: CustomBundler.settings["gem.github_username"], banner: "Set your username on GitHub", desc: "Fill in GitHub username on README so that you don't have to do it manually. Set a default with `bundle config set --global gem.github_username <your_username>`."
    ...
    method_option "force", type: :boolean, default: false, banner: "Forces cleaning up unused gems even if CustomBundler is configured to use globally installed gems. As a consequence, removes all system gems except for the ones in the current application."
    def clean
    ...
    desc "env", "Print information about the environment CustomBundler is running under"
    def env
      Env.write($stdout)
    end
    ...
    long_desc <<-D
      ...
      missing dependencies are detected, CustomBundler prints them and exits status 1.
      Otherwise, CustomBundler prints a success message and exits with a status of 0.
    D
    method_option "gemfile", type: :string, banner: "Use the specified gemfile instead of Gemfile"
    method_option "quiet", type: :boolean, banner: "Only output warnings and errors."
    def doctor
    ...
    desc "issue", "Learn how to report an issue in CustomBundler"
    def issue
    ...
    if CustomBundler.feature_flag.plugins?
    ...
    def print_command
      return unless CustomBundler.ui.debug?
      cmd = current_command
      ...
      CustomBundler.ui.info "Running `#{command * " "}` with bundler #{CustomBundler::VERSION}"
    end

    def warn_on_outdated_bundler
      return if CustomBundler.settings[:disable_version_check]
      ...
      cache_path = CustomBundler.user_cache.join("compact_index", remote.cache_slug)
      latest = CustomBundler::CompactIndexClient.new(cache_path).latest_version("bundler")
      return unless latest

      current = Gem::Version.new(VERSION)
      return if current >= latest

      CustomBundler.ui.warn \
    ...
    def print_remembered_flag_deprecation(flag_name, option_name, option_value)
      ...
      removed_message =
        ...
        "#{option_value}`, and stop using this flag"
      CustomBundler::SharedHelpers.major_deprecation 2, message, removed_message: removed_message
    end
  end
end
