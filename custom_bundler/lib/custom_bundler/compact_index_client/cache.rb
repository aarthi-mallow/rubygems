    ...
    class Cache
      ...
      def info(name, remote_checksum = nil)
        ...
        if remote_checksum && remote_checksum != SharedHelpers.checksum_for_file(path, :MD5)
          ...
          CustomBundler::CompactIndexClient.debug { "update skipped info/#{name} (#{remote_checksum ? "versions index checksum is nil" : "versions index checksum matches local"})" }
          read(path)
        end
      end
      ...
      def fetch(remote_path, path, etag_path)
        if already_fetched?(remote_path)
          CustomBundler::CompactIndexClient.debug { "already fetched #{remote_path}" }
        else
          CustomBundler::CompactIndexClient.debug { "fetching #{remote_path}" }
