...
module CustomBundler
  class Dsl
    ...
    VALID_PLATFORMS = CustomBundler::Dependency::PLATFORM_MAP.keys.freeze
    ...
    def eval_gemfile(gemfile, contents = nil)
      ...
      contents ||= CustomBundler.read_file(@gemfile.to_s)
    ...
    def gemspec(opts = nil)
      ...
      gemspecs = Gem::Util.glob_files_in_dir("{,*}.gemspec", expanded_path).map {|g| CustomBundler.load_gemspec(g) }.compact
      ...
      when 1
        ...
        spec = specs.find {|s| s.match_platform(CustomBundler.local_platform) } || specs.first
    ...
    def gem(name, *args)
      ...
      if current = @dependencies.find {|d| d.name == dep.name }
        if current.requirement != dep.requirement
          ...
          if gemspec_dep
            ...
            unless current_requirement_open
              CustomBundler.ui.warn "A gemspec development dependency (#{gemspec_dep.name}, #{gemspec_dep.requirement}) is being overridden by a Gemfile dependency (#{gemfile_dep.name}, #{gemfile_dep.requirement}).\n" \
        ...
        if current.gemspec_dev_dep?
          ...
          CustomBundler.ui.warn "Your Gemfile lists the gem #{current.name} (#{current.requirement}) more than once.\n" \
    ...
    def validate_keys(command, opts, valid_keys)
      ...
      message << " You may be able to resolve this by upgrading CustomBundler to the newest version."
    ...
    def implicit_global_source_warning
      CustomBundler::SharedHelpers.major_deprecation 2, "This Gemfile does not include an explicit global source. " \
    ...
    def multiple_global_source_warning
      if CustomBundler.feature_flag.bundler_3_mode?
    ...
    class DSLError < GemfileError
      ...
      def to_s
        @to_s ||= begin
          ...
          m = String.new("\n[!] ")
          m << description
          m << ". CustomBundler cannot continue.\n"
    ...
    def gemfile_root
      @gemfile ||= CustomBundler.default_gemfile
      @gemfile.dirname
    end
  end
end
