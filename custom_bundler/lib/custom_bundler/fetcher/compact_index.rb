    ...
    class CompactIndex < Base
      ...
      def available?
        unless SharedHelpers.md5_available?
          CustomBundler.ui.debug("FIPS mode is enabled, bundler can't use the CompactIndex API")
          return nil
        end
        # Read info file checksums out of /versions, so we can know if gems are up to date
        compact_index_client.available?
      rescue CompactIndexClient::Updater::MismatchedChecksumError => e
        CustomBundler.ui.debug(e.message)
        nil
      end
      ...
      def bundle_worker(func = nil)
        @bundle_worker ||= begin
          ...
          CustomBundler::Worker.new(CustomBundler.settings.processor_count, worker_name, func)
        end
      ...
      def cache_path
        CustomBundler.user_cache.join("compact_index", remote.cache_slug)
      end

      def client_fetcher
        ClientFetcher.new(self, CustomBundler.ui)
      end
      ...
      ClientFetcher = Struct.new(:fetcher, :ui) do
        def call(path, headers)
          ...
          raise unless CustomBundler.feature_flag.allow_offline_install? && headers["If-None-Match"]
