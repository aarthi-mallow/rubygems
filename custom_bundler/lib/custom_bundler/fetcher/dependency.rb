  ...
  class Fetcher
    class Dependency < Base
      ...
      def specs(gem_names, full_dependency_list = [], last_spec_list = [])
        ...
        spec_list, deps_list = CustomBundler::Retry.new("dependency api", FAIL_ERRORS).attempts do
        ...
        CustomBundler.ui.debug "could not fetch from the dependency API, trying the full index"
        nil
      end

      def dependency_specs(gem_names)
        CustomBundler.ui.debug "Query Gemcutter Dependency Endpoint API: #{gem_names.join(",")}"
      ...
      def unmarshalled_dep_gems(gem_names)
        ...
        gem_names.each_slice(Source::Rubygems::API_REQUEST_SIZE) do |names|
          ...
          gem_list.concat(CustomBundler.safe_load_marshal(marshalled_deps))
        end
        gem_list
      end
