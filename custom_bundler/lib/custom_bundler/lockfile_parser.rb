...
module CustomBundler
  class LockfileParser
    ...
    def self.bundled_with
      lockfile = CustomBundler.default_lockfile
    ...
    TYPES = {
      GIT => CustomBundler::Source::Git,
      GEM => CustomBundler::Source::Rubygems,
      PATH => CustomBundler::Source::Path,
      PLUGIN => CustomBundler::Plugin,
    }.freeze
    ...
    def parse_dependency(line)
      ...
      dep = CustomBundler::Dependency.new(name, version)
      ...
      if pinned && dep.name != "bundler"
        ...
        if version && version.size == 1 && version.first =~ /^\s*= (.+)\s*$/ && dep.source.is_a?(CustomBundler::Source::Path)
