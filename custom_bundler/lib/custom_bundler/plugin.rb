...
module CustomBundler
  module Plugin
    ...
    def install(names, options)
      ...
      specs_to_delete.each_value {|spec| CustomBundler.rm_rf(spec.full_gem_path) }
    ...
    def uninstall(names, options)
      if names.empty? && !options[:all]
        CustomBundler.ui.error "No plugins to uninstall. Specify at least 1 plugin to uninstall.\n"\
      ...
      if names.any?
        names.each do |name|
          if index.installed?(name)
            ...
            CustomBundler.rm_rf(path) if index.installed_in_plugin_root?(name)
            index.unregister_plugin(name)
            CustomBundler.ui.info "Uninstalled plugin #{name}"
          else
            CustomBundler.ui.error "Plugin #{name} is not installed \n"
          end
        end
      else
        CustomBundler.ui.info "No plugins installed"
      end
    end
    ...
    def gemfile_install(gemfile = nil, &inline)
      CustomBundler.settings.temporary(frozen: false, deployment: false) do
        builder = DSL.new
        if block_given?
      ...
      unless e.is_a?(GemfileError)
        CustomBundler.ui.error "Failed to install plugin: #{e.message}\n  #{e.backtrace[0]}"
      end
      raise
    end
    ...
    # Otherwise, points to global root, in CustomBundler.user_bundle_path("plugin")
    def root
    ...
    def local_root
      CustomBundler.app_config_path.join("plugin")
    end

    # The global directory root for all plugin related data
    def global_root
      CustomBundler.user_bundle_path("plugin")
    end
    ...
    def add_hook(event, &block)
      unless Events.defined_event?(event)
        raise ArgumentError, "Event '#{event}' not defined in CustomBundler::Plugin::Events"
      end
    ...
    def hook(event, *args, &arg_blk)
      return unless CustomBundler.feature_flag.plugins?
      unless Events.defined_event?(event)
        raise ArgumentError, "Event '#{event}' not defined in CustomBundler::Plugin::Events"
      end
    ...
    def save_plugin(name, spec, optional_plugin = false)
      ...
      CustomBundler.ui.info "Installed plugin #{name}" if installed
    ...
    def load_plugin(name)
      ...
      CustomBundler.ui.error "Failed loading plugin #{name}: #{e.message}"
      raise
    end
