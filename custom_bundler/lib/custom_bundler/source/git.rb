    ...
    class Git < Path
      ...
      def install_path
        @install_path ||= begin
          ...
          CustomBundler.install_path.join(git_scope)
        end
      end
      ...
      def local_override!(path)
        ...
        path = path.expand_path(CustomBundler.root) unless path.relative?

        unless branch || CustomBundler.settings[:disable_local_branch_check]
        ...
        if current_branch != branch && !CustomBundler.settings[:disable_local_branch_check]
        ...
        if !CustomBundler.settings[:disable_local_revision_check] && changed && !@unlocked && !git_proxy.contains?(cached_revision)
      ...
      def install(spec, options = {})
        ...
        if (requires_checkout? && !@copied) || force
          CustomBundler.ui.debug "  * Checking out revision: #{ref}"
          git_proxy.copy_to(install_path, submodules)
          serialize_gemspecs_in(install_path)
          @copied = true
        end

        generate_bin_options = { disable_extensions: !CustomBundler.rubygems.spec_missing_extensions?(spec), build_args: options[:build_args] }
      ...
      def cache(spec, custom_path = nil)
        ...
        return unless CustomBundler.feature_flag.cache_all?
      ...
      def cache_path
        @cache_path ||= if CustomBundler.feature_flag.global_gem_cache?
          CustomBundler.user_cache
        else
          CustomBundler.bundle_path.join("cache", "bundler")
      ...
      def serialize_gemspecs_in(destination)
        destination = destination.expand_path(CustomBundler.root) if destination.relative?
        Dir["#{destination}/#{@glob}"].each do |spec_path|
          # Evaluate gemspecs and cache the result. Gemspecs
          # in git might require git or other dependencies.
          # The gemspecs we cache should already be evaluated.
          spec = CustomBundler.load_gemspec(spec_path)
          next unless spec
          CustomBundler.rubygems.set_installed_by_version(spec)
          CustomBundler.rubygems.validate(spec)
      ...
      def uri_hash
        ...
        CustomBundler::Digest.sha1(input)
      end
      ...
      def fetch
        ...
        raise unless CustomBundler.feature_flag.allow_offline_install?
        CustomBundler.ui.warn "Using cached git data because of network errors:\n#{e}"
      end
      ...
      def override_for(path)
        CustomBundler.settings.local_overrides.key(path)
      end
    end
  end
end
