...
module CustomBundler
  ...
  module SharedHelpers
    ...
    def default_bundle_dir
      ...
      global_bundle_dir = CustomBundler.user_home.join(".bundle")
    ...
    def chdir(dir, &blk)
      CustomBundler.rubygems.ext_lock.synchronize do
        Dir.chdir dir, &blk
      end
    end
    ...
    # @yield path
    #
    # @raise [CustomBundler::PermissionError] if Errno:EACCES is raised in the
    #   given block
    # @raise [CustomBundler::TemporaryResourceError] if Errno:EAGAIN is raised in the
    #   given block
    #
    # @example
    ...
    #   end
    #
    # @see {CustomBundler::PermissionError}
    ...
    def major_deprecation(major_version, message, removed_message: nil, print_caller_location: false)
      ...
      bundler_major_version = CustomBundler.bundler_major_version
      ...
      CustomBundler.ui.warn("[DEPRECATED] #{message}")
    end
    ...
    def ensure_same_dependencies(spec, old_deps, new_deps)
      ...
      CustomBundler.ui.debug "#{spec.full_name} from #{spec.remote} has either corrupted API or lockfile dependencies" \
    ...
    def pretty_dependency(dep)
      ...
      if dep.is_a?(CustomBundler::Dependency)
    ...
    def relative_gemfile_path
      relative_path_to(CustomBundler.default_gemfile)
    end

    def relative_lockfile_path
      relative_path_to(CustomBundler.default_lockfile)
    end
    ...
    def validate_bundle_path
      path_separator = CustomBundler.rubygems.path_separator
      return unless CustomBundler.bundle_path.to_s.split(path_separator).size > 1
      message = "Your bundle path contains text matching #{path_separator.inspect}, " \
                "which is the path separator for your system. CustomBundler cannot " \
                "function correctly when the Bundle path contains the " \
                "system's PATH separator. Please change your " \
                "bundle path to not match #{path_separator.inspect}." \
                "\nYour current bundle path is '#{CustomBundler.bundle_path}'."
      raise CustomBundler::PathError, message
    end
    ...
    def set_bundle_variables
      CustomBundler::SharedHelpers.set_env "BUNDLE_BIN_PATH", bundle_bin_path
      CustomBundler::SharedHelpers.set_env "BUNDLE_GEMFILE", find_gemfile.to_s
      CustomBundler::SharedHelpers.set_env "BUNDLER_VERSION", CustomBundler::VERSION
      CustomBundler::SharedHelpers.set_env "BUNDLER_SETUP", File.expand_path("setup", __dir__)
    end
    ...
    def bundle_bin_path
      ...
      exe_file = CustomBundler.rubygems.bin_path("bundler", "bundle", VERSION) unless File.exist?(exe_file)
    ...
    def set_path
      ...
      paths.unshift "#{CustomBundler.bundle_path}/bin"
      CustomBundler::SharedHelpers.set_env "PATH", paths.uniq.join(File::PATH_SEPARATOR)
    end
    ...
    def set_rubyopt
      ...
      CustomBundler::SharedHelpers.set_env "RUBYOPT", rubyopt.join(" ")
    end

    def set_rubylib
      rubylib = (ENV["RUBYLIB"] || "").split(File::PATH_SEPARATOR)
      rubylib.unshift bundler_ruby_lib unless RbConfig::CONFIG["rubylibdir"] == bundler_ruby_lib
      CustomBundler::SharedHelpers.set_env "RUBYLIB", rubylib.uniq.join(File::PATH_SEPARATOR)
    end
    ...
    def clean_load_path
      loaded_gem_paths = CustomBundler.rubygems.loaded_gem_paths
    ...
    def prints_major_deprecations?
      ...
      return false if CustomBundler.settings[:silence_deprecations]
      require_relative "deprecate"
      return false if CustomBundler::Deprecate.skip
      true
    end
