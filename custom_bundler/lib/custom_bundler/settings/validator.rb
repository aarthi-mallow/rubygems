    ...
    class Validator
      class Rule
        ...
        def set(settings, key, value, *reasons)
          ...
          reasons.unshift @description
          CustomBundler.ui.info "Setting `#{key}` to #{value.inspect}, since #{reasons.join(", ")}"
          if value.nil?
        ...
        def k(key)
          CustomBundler.settings.key_for(key)
        end
      end
      ...
      rule %w[path], "relative paths are expanded relative to the current working directory" do |key, value, settings|
        ...
        next if !path.relative? || !CustomBundler.feature_flag.path_relative_to_cwd?
