...
module CustomBundler
  ...
  class SelfManager
    ...
    def install_locked_bundler_and_restart_with_it_if_needed
      ...
      if restart_version == lockfile_version
        CustomBundler.ui.info \
          "CustomBundler #{current_version} is running, but your lockfile was generated with #{lockfile_version}. " \
          "Installing CustomBundler #{lockfile_version} and restarting using that version."
      else
        CustomBundler.ui.info \
          "CustomBundler #{current_version} is running, but your configuration was #{restart_version}. " \
          "Installing CustomBundler #{restart_version} and restarting using that version."
      end
    ...
    def update_bundler_and_restart_with_it_if_needed(target)
      ...
      CustomBundler.ui.info "Updating bundler to #{version}."
    ...
    def install_and_restart_with(version)
      ...
      if spec.nil?
        CustomBundler.ui.warn "Your lockfile is locked to a version of bundler (#{lockfile_version}) that doesn't exist at https://rubygems.org/. Going on using #{current_version}"
        return
      end

      install(spec)
    rescue StandardError => e
      CustomBundler.ui.trace e
      CustomBundler.ui.warn "There was an error installing the locked bundler version (#{lockfile_version}), rerun with the `--verbose` flag for more details. Going on using bundler #{current_version}."
    else
      restart_with(version)
    end
    ...
    def restart_with(version)
      ...
      # CustomBundler specs need some stuff to be required before CustomBundler starts
      # running, for example, for faking the compact index API. However, these
      # flags are lost when we reexec to a different version of CustomBundler. In the
      ...
      CustomBundler.with_original_env do
        Kernel.exec(
    ...
    def needs_switching?
      autoswitching_applies? &&
        ...
        !updating? &&
        CustomBundler.settings[:version] != "system"
    end

    def autoswitching_applies?
      ENV["BUNDLER_VERSION"].nil? &&
        CustomBundler.rubygems.supports_bundler_trampolining? &&
    ...
    def local_specs
      @local_specs ||= CustomBundler::Source::Rubygems.new("allow_local" => true).specs.select {|spec| spec.name == "bundler" }
    end

    def remote_specs
      @remote_specs ||= begin
        source = CustomBundler::Source::Rubygems.new("remotes" => "https://rubygems.org")
        source.remote!
    ...
    def installed?
      CustomBundler.configure

      CustomBundler.rubygems.find_bundler(restart_version.to_s)
    end

    def current_version
      @current_version ||= Gem::Version.new(CustomBundler::VERSION)
    end

    def lockfile_version
      return @lockfile_version if defined?(@lockfile_version)

      parsed_version = CustomBundler::LockfileParser.bundled_with
    ...
    def restart_version
      ...
      @restart_version = Gem::Version.new(CustomBundler.settings[:version])
    rescue ArgumentError
