...
module CustomBundler
  ...
  class CompactIndexClient
    ...
    def names
      CustomBundler::CompactIndexClient.debug { "names" }
      @parser.names
    end

    def versions
      CustomBundler::CompactIndexClient.debug { "versions" }
      @parser.versions
    end

    def dependencies(names)
      CustomBundler::CompactIndexClient.debug { "dependencies(#{names})" }
      names.map {|name| info(name) }
    end

    def info(name)
      CustomBundler::CompactIndexClient.debug { "info(#{name})" }
      @parser.info(name)
    end

    def latest_version(name)
      CustomBundler::CompactIndexClient.debug { "latest_version(#{name})" }
      @parser.info(name).map {|d| Gem::Version.new(d[INFO_VERSION]) }.max
    end

    def available?
      CustomBundler::CompactIndexClient.debug { "available?" }
      @parser.available?
    end

    def reset!
      CustomBundler::CompactIndexClient.debug { "reset!" }
      @cache.reset!
    end
  end
end
