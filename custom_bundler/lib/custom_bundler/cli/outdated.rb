...
module CustomBundler
  class CLI::Outdated
    ...
    def initialize(options, gems)
      ...
      # sense otherwise.
      @strict = options["filter-strict"] || CustomBundler::CLI::Common.patch_level_options(options).any?
    end
    ...
    def run
      ...
      gems.each do |gem_name|
        CustomBundler::CLI::Common.select_spec(gem_name)
      end

      CustomBundler.definition.validate_runtime!
      current_specs = CustomBundler.ui.silence { CustomBundler.definition.resolve }

      current_dependencies = CustomBundler.ui.silence do
        CustomBundler.load.dependencies.map {|dep| [dep.name, dep] }.to_h
      end
      ...
      definition = if gems.empty? && sources.empty?
        ...
        CustomBundler.definition(gems: gems, sources: sources)
      end

      CustomBundler::CLI::Common.configure_gem_version_promoter(
        CustomBundler.definition,
      ...
      if options[:parseable]
        CustomBundler.ui.silence(&definition_resolution)
      else
      ...
      if outdated_gems.empty?
        unless options[:parseable]
          CustomBundler.ui.info(nothing_outdated_message)
        end
      else
    ...
    def print_gem(current_spec, active_spec, dependency, groups)
      ...
      CustomBundler.ui.info output_message.rstrip
    end
    ...
    def gem_column_for(current_spec, active_spec, dependency, groups)
      ...
      ret_val << loaded_from_for(active_spec).to_s if CustomBundler.ui.debug?
      ret_val
    end

    def check_for_deployment_mode!
      return unless CustomBundler.frozen_bundle?
      suggested_command = if CustomBundler.settings.locations("frozen").keys.&([:global, :local]).any?
        "bundle config unset frozen"
      elsif CustomBundler.settings.locations("deployment").keys.&([:global, :local]).any?
        "bundle config unset deployment"
      end
      raise ProductionError, "You are trying to check outdated gems in " \
        "deployment mode. Run `bundle outdated` elsewhere.\n" \
        "\nIf this is a development machine, remove the " \
        "#{CustomBundler.default_gemfile} freeze" \
    ...
    def print_indented(matrix)
      ...
      CustomBundler.ui.info justify(header, column_sizes)

      data.sort_by! {|row| row[0] }

      data.each do |row|
        CustomBundler.ui.info justify(row, column_sizes)
      end
    end

    def table_header
      header = ["Gem", "Current", "Latest", "Requested", "Groups"]
      header << "Path" if CustomBundler.ui.debug?
      header
    end
