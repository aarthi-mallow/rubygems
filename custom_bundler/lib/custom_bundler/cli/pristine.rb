...
module CustomBundler
  class CLI::Pristine
    ...
    def run
      ...
      installer = CustomBundler::Installer.new(CustomBundler.root, definition)
        ...
        installed_specs = definition.specs.reject do |spec|
          ...
          when Source::Rubygems
            ...
            unless File.exist?(cached_gem)
              CustomBundler.ui.error("Failed to pristine #{gem_name}. Cached gem #{cached_gem} does not exist.")
              next
            end
          ...
          when Source::Git
            if source.local?
              CustomBundler.ui.warn("Cannot pristine #{gem_name}. Gem is locally overridden.")
              next
            end
            ...
            CustomBundler.ui.warn("Cannot pristine #{gem_name}. Gem is sourced from local path.")
            next
          end
