...
module CustomBundler
  class CLI::List
    ...
    def run
      ...
      return CustomBundler.ui.info "No gems in the Gemfile" if specs.empty?

      return specs.each {|s| CustomBundler.ui.info s.name } if @options["name-only"]
      return specs.each {|s| CustomBundler.ui.info s.full_gem_path } if @options["paths"]

      CustomBundler.ui.info "Gems included by the bundle:"

      specs.each {|s| CustomBundler.ui.info "  * #{s.name} (#{s.version}#{s.git_version})" }

      CustomBundler.ui.info "Use `bundle info` to print more detailed information about a gem"
    end
