...
module CustomBundler
  class Runtime
    ...
    def setup(*groups)
      ...
      CustomBundler.rubygems.replace_entrypoints(specs)

      # Activate the specs
      load_paths = specs.map do |spec|
        check_for_activated_spec!(spec)

        CustomBundler.rubygems.mark_loaded(spec)
        ...
        begin
          ...
          Array(dep.autorequire || dep.name).each do |file|
            ...
            begin
              ...
              raise CustomBundler::GemRequireError.new e,
    ...
    def cache(custom_path = nil, local = false)
      cache_path = CustomBundler.app_cache(custom_path)
      SharedHelpers.filesystem_access(cache_path) do |p|
        FileUtils.mkdir_p(p)
      end unless File.exist?(cache_path)

      CustomBundler.ui.info "Updating files in #{CustomBundler.settings.app_cache_path}"

      specs_to_cache = if CustomBundler.settings[:cache_all_platforms]
        ...
        begin
          ...
          if local
            CustomBundler.ui.warn "Some gems seem to be missing from your #{CustomBundler.settings.app_cache_path} directory."
          end
      ...
      prune_cache(cache_path) unless CustomBundler.settings[:no_prune]
    end
    ...
    def clean(dry_run = false)
      ...
      CustomBundler.rubygems.add_default_gems_to(specs).values.each do |spec|
        spec_gem_paths << spec.full_gem_path
        # need to check here in case gems are nested like for the rails git repo
        md = %r{(.+bundler/gems/.+-[a-f0-9]{7,12})}.match(spec.full_gem_path)
        spec_git_paths << md[1] if md
        spec_gem_executables << spec.executables.collect do |executable|
          e = "#{CustomBundler.rubygems.gem_bindir}/#{executable}"
          [e, "#{e}.bat"]
        end
        spec_cache_paths << spec.cache_file
        spec_gemspec_paths << spec.spec_file
        spec_extension_paths << spec.extension_dir if spec.respond_to?(:extension_dir)
        spec_git_cache_dirs << spec.source.cache_path.to_s if spec.source.is_a?(CustomBundler::Source::Git)
      end
      spec_gem_paths.uniq!
    ...
    def prune_gem_cache(resolve, cache_path)
      ...
      cached = cached.delete_if do |path|
        spec = CustomBundler.rubygems.spec_from_gem path

        resolve.any? do |s|
          s.name == spec.name && s.version == spec.version && !s.source.is_a?(CustomBundler::Source::Git)
        end
      end

      if cached.any?
        CustomBundler.ui.info "Removing outdated .gem files from #{CustomBundler.settings.app_cache_path}"

        cached.each do |path|
          CustomBundler.ui.info "  * #{File.basename(path)}"
          File.delete(path)
        end
      end
    end
    ...
    def prune_git_and_path_cache(resolve, cache_path)
      ...
      if cached.any?
        CustomBundler.ui.info "Removing outdated git and path gems from #{CustomBundler.settings.app_cache_path}"
    ...
    def setup_manpath
      ...
      CustomBundler::SharedHelpers.set_env "MANPATH", manuals.concat(
    ...
    def remove_dir(dir, dry_run)
      ...
      if dry_run
        CustomBundler.ui.info "Would have removed #{output}"
      else
        CustomBundler.ui.info "Removing #{output}"
        FileUtils.rm_rf(dir)
      end
    ...
    def check_for_activated_spec!(spec)
      return unless activated_spec = CustomBundler.rubygems.loaded_specs(spec.name)
