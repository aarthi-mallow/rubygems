...
module CustomBundler
  class Source
    ...
    def version_message(spec, locked_spec = nil)
      ...
      if locked_spec
        ...
        if locked_spec_version && spec.version != locked_spec_version
          message += CustomBundler.ui.add_color(" (was #{locked_spec_version})", version_color(spec.version, locked_spec_version))
        end
      end
    ...
    def path?
      instance_of?(CustomBundler::Source::Path)
    end

    def extension_cache_path(spec)
      return unless CustomBundler.feature_flag.global_gem_cache?
      return unless source_slug = extension_cache_slug(spec)
      CustomBundler.user_cache.join(
        "extensions", Gem::Platform.local.to_s, CustomBundler.ruby_scope,
    ...
    def print_using_message(message)
      if !message.include?("(was ")
        CustomBundler.ui.debug message
      else
