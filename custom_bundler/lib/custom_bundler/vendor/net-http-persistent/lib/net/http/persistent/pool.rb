class Gem::Net::HTTP::Persistent::Pool < CustomBundler::ConnectionPool # :nodoc:
  ...
  def checkin net_http_args
    if net_http_args.is_a?(Hash) && net_http_args.size == 1 && net_http_args[:force]
      # CustomBundler::ConnectionPool 2.4+ calls `checkin(force: true)` after fork.
      ...
      raise CustomBundler::ConnectionPool::Error, 'no connections are checked out' if
        stack.empty?
