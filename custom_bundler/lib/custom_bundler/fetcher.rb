...
module CustomBundler
  ...
  class Fetcher
    ...
    self.redirect_limit = CustomBundler.settings[:redirect] # How many redirects to allow in one request
    self.api_timeout    = CustomBundler.settings[:timeout] # How long to wait for each API call
    self.max_retries    = CustomBundler.settings[:retry] # How many retries for the API call
    ...
    def fetch_spec(spec)
      ...
      spec = if uri.scheme == "file"
        ...
        CustomBundler.safe_load_marshal CustomBundler.rubygems.inflate(Gem.read_binary(path))
      elsif cached_spec_path = gemspec_cached_path(spec_file_name)
        CustomBundler.load_gemspec(cached_spec_path)
      else
        CustomBundler.safe_load_marshal CustomBundler.rubygems.inflate(downloader.fetch(uri).body)
      end
    ...
    def specs_with_retry(gem_names, source)
      CustomBundler::Retry.new("fetcher", FAIL_ERRORS).attempts do
    ...
    def specs(gem_names, source)
      ...
      CustomBundler.ui.info "" if gem_names && api_fetcher? # newline after dots
      raise
    end

    def user_agent
      @user_agent ||= begin
        ruby = CustomBundler::RubyVersion.system

        agent = String.new("bundler/#{CustomBundler::VERSION}")
        ...
        agent << " options/#{CustomBundler.settings.all.join(",")}"
        ...
        extra_ua = CustomBundler.settings[:user_agent]
    ...
    def available_fetchers
      if CustomBundler::Fetcher.disable_endpoint
        [Index]
      elsif remote_uri.scheme == "file"
        CustomBundler.ui.debug("Using a local server, bundler won't use the CompactIndex API")
        [Index]
      else
    ...
    def cis
      @cis ||= CustomBundler::CIDetector.ci_strings
    end

    def connection
      @connection ||= begin
        needs_ssl = remote_uri.scheme == "https" ||
                    CustomBundler.settings[:ssl_verify_mode] ||
                    CustomBundler.settings[:ssl_client_cert]
        ...
        if remote_uri.scheme == "https"
          con.verify_mode = (CustomBundler.settings[:ssl_verify_mode] ||
            OpenSSL::SSL::VERIFY_PEER)
          con.cert_store = bundler_cert_store
        end

        ssl_client_cert = CustomBundler.settings[:ssl_client_cert] ||
    ...
    def gemspec_cached_path(spec_file_name)
      paths = CustomBundler.rubygems.spec_cache_dirs.map {|dir| File.join(dir, spec_file_name) }
    ...
    def bundler_cert_store
      ...
      ssl_ca_cert = CustomBundler.settings[:ssl_ca_cert] ||
