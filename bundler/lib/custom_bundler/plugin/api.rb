...
module CustomBundler
  ...
  # the plugins to use.
  #
  # For plugins to be independent of the CustomBundler internals they shall limit their
  # interactions to methods of this class only. This will save them from breaking
  # when some internal change.
  #
  # Currently we are delegating the methods defined in CustomBundler class to
  # itself. So, this class acts as a buffer.
  #
  # If there is some change in the CustomBundler class that is incompatible to its
  ...
  module Plugin
    class API
      ...
      def self.source(source, cls = self)
        cls.send :include, CustomBundler::Plugin::API::Source
      ...
      def tmp(*names)
        CustomBundler.tmp(["plugin", *names].join("-"))
      end

      def method_missing(name, *args, &blk)
        return CustomBundler.send(name, *args, &blk) if CustomBundler.respond_to?(name)
      ...
      def respond_to_missing?(name, include_private = false)
        SharedHelpers.respond_to?(name, include_private) ||
          CustomBundler.respond_to?(name, include_private) || super
      end
    end
  end
end
