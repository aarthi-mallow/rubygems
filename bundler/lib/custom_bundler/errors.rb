...
module CustomBundler
  ...
  class ChecksumMismatchError < SecurityError
    ...
    def message
      <<~MESSAGE
        CustomBundler found mismatched checksums. This is a potential security risk.
  ...
  class InsecureInstallPathError < BundlerError
    ...
    def message
      "The installation path is insecure. CustomBundler cannot continue.\n" \
      "#{@path} is world-writable (without sticky bit).\n" \
      "CustomBundler cannot safely replace gems in world-writeable directories due to potential vulnerabilities.\n" \
  ...
  class CorruptBundlerInstallError < BundlerError
    ...
    def message
      "The running version of CustomBundler (#{CustomBundler::VERSION}) does not match the version of the specification installed for it (#{@loaded_spec.version}). " \
      "This can be caused by reinstalling Ruby without removing previous installation, leaving around an upgraded default version of CustomBundler. " \
