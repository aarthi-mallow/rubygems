...
module CustomBundler
  class Env
    ...
    def self.report(options = {})
      ...
      append_formatted_table("CustomBundler Build Metadata", BuildMetadata.to_h, out)

      unless CustomBundler.settings.all.empty?
        out << "\n## CustomBundler settings\n\n```\n"
        CustomBundler.settings.all.each do |setting|
          out << setting << "\n"
          CustomBundler.settings.pretty_values_for(setting).each do |line|
            out << "  " << line << "\n"
          end
        end
        out << "```\n"
      ...
      if print_gemfile
        gemfiles = [CustomBundler.default_gemfile]
        begin
          gemfiles = CustomBundler.definition.gemfiles
        ...
        out << "\n### #{SharedHelpers.relative_path_to(CustomBundler.default_lockfile)}\n\n"
        out << "```\n" << read_file(CustomBundler.default_lockfile).chomp << "\n```\n"
      end

      if print_gemspecs
        dsl = Dsl.new.tap {|d| d.eval_gemfile(CustomBundler.default_gemfile) }
    ...
    def self.read_file(filename)
      CustomBundler.read_file(filename.to_s).strip
    rescue Errno::ENOENT
      "<No #{filename} found>"
    ...
    def self.git_version
      CustomBundler::Source::Git::GitProxy.new(nil, nil).full_version
    rescue CustomBundler::Source::Git::GitNotInstalledError
      "not installed"
    end

    def self.version_of(script)
      return "not installed" unless CustomBundler.which(script)
      `#{script} --version`.chomp
    end

    def self.chruby_version
      return "not installed" unless CustomBundler.which("chruby-exec")
    ...
    def self.environment
      ...
      out << ["CustomBundler", CustomBundler::VERSION]
      ...
      %w[rubygems-bundler open_gem].each do |name|
        specs = CustomBundler.rubygems.find_name(name)
