    ...
    class Git
      ...
      class GitNotAllowedError < GitError
        def initialize(command)
          msg = String.new
          msg << "CustomBundler is trying to run `#{command}` at runtime. You probably need to run `bundle install`. However, "
          msg << "this error message could probably be more useful. Please submit a ticket at https://github.com/rubygems/rubygems/issues/new?labels=CustomBundler&template=bundler-related-issue.md "
      ...
      class GitProxy
        ...
        def checkout
          ...
          CustomBundler.ui.info "Fetching #{credential_filtered_uri}"
        ...
        def copy_to(destination, submodules = false)
          unless File.exist?(destination.join(".git"))
            begin
              ...
              raise GitError, "CustomBundler could not install a gem because it needs to " \
        ...
        def git_remote_fetch(args)
          ...
          CustomBundler::Retry.new("`#{command_with_no_credentials}` at #{path}", [MissingGitRevisionError]).attempts do
        ...
        def clone_needs_extra_fetch?
          ...
          CustomBundler::Retry.new("`#{command_with_no_credentials}`", [MissingGitRevisionError]).attempts do
        ...
        def git_retry(*command, dir: nil)
          ...
          CustomBundler::Retry.new("`#{command_with_no_credentials}` at #{dir || SharedHelpers.pwd}").attempts do
            git(*command, dir: dir)
          end
        end
        ...
        def configured_uri
          if /https?:/.match?(uri)
            remote = Gem::URI(uri)
            config_auth = CustomBundler.settings[remote.to_s] || CustomBundler.settings[remote.host]
        ...
        def allow?
          ...
          raise GitNotInstalledError.new if allowed && !CustomBundler.git_present?
        ...
        def redact_and_check_presence(command)
          raise GitNotInstalledError.new unless CustomBundler.git_present?
        ...
        def run_command(*command, dir: nil)
          ...
          CustomBundler.ui.warn err unless err.empty?
        ...
        def capture3_args_for(cmd, dir)
          ...
          if CustomBundler.feature_flag.bundler_3_mode? || supports_minus_c?
            ["git", "-C", dir.to_s, *cmd]
          else
