...
module CustomBundler
  class Source
    class Rubygems < Source
      ...
      def caches
        @caches ||= [cache_path, *CustomBundler.rubygems.gem_cache]
      end
      ...
      def install(spec, options = {})
        ...
        return if CustomBundler.settings[:no_install]

        install_path = rubygems_dir
        bin_path     = CustomBundler.system_bindir

        require_relative "../rubygems_gem_installer"

        installer = CustomBundler::RubyGemsGemInstaller.at(
          path,
          security_policy: CustomBundler.rubygems.security_policies[CustomBundler.settings["trust-policy"]],
        ...
        CustomBundler.ui.confirm message
      ...
      def cache(spec, custom_path = nil)
        cached_path = CustomBundler.settings[:cache_all_platforms] ? fetch_gem_if_possible(spec) : cached_gem(spec)
        raise GemNotFound, "Missing gem file '#{spec.file_name}'." unless cached_path
        return if File.dirname(cached_path) == CustomBundler.app_cache.to_s
        CustomBundler.ui.info "  * #{File.basename(cached_path)}"
        FileUtils.cp(cached_path, CustomBundler.app_cache(custom_path))
      rescue Errno::EACCES => e
        CustomBundler.ui.debug(e)
      ...
      def cached_built_in_gem(spec)
        ...
        if cached_path.nil?
          ...
          if remote_spec
            ...
            CustomBundler.ui.warn "#{spec.full_name} is built in to Ruby, and can't be cached because your Gemfile doesn't have any sources that contain it."
          end
        end
        cached_path
      end
      ...
      def remote_fetchers
        @remote_fetchers ||= remotes.to_h do |uri|
          ...
          [remote, CustomBundler::Fetcher.new(remote)]
        end.freeze
      end
      ...
      def double_check_for(unmet_dependency_names)
        ...
        CustomBundler.ui.debug "Double checking for #{unmet_dependency_names || "all specs (due to the size of the request)"} in #{self}"
      ...
      def installed_specs
        @installed_specs ||= Index.build do |idx|
          CustomBundler.rubygems.installed_specs.reverse_each do |spec|
            spec.source = self
            if CustomBundler.rubygems.spec_missing_extensions?(spec, false)
              CustomBundler.ui.debug "Source #{self} is ignoring #{spec} because it is missing extensions"
              next
            end
            idx << spec
          end
      ...
      def default_specs
        @default_specs ||= Index.build do |idx|
          CustomBundler.rubygems.default_specs.each do |spec|
            spec.source = self
            idx << spec
          end
        end
      ...
      def cached_specs
        @cached_specs ||= begin
          ...
          Dir["#{cache_path}/*.gem"].each do |gemfile|
            s ||= CustomBundler.rubygems.spec_from_gem(gemfile)
            s.source = self
            idx << s
          end
      ...
      def fetch_names(fetchers, dependency_names, index)
        fetchers.each do |f|
          if dependency_names
            CustomBundler.ui.info "Fetching gem metadata from #{URICredentialsFilter.credential_filtered_uri(f.uri)}", CustomBundler.ui.debug?
            index.use f.specs_with_retry(dependency_names, self)
            CustomBundler.ui.info "" unless CustomBundler.ui.debug? # new line now that the dots are over
          else
            CustomBundler.ui.info "Fetching source index from #{URICredentialsFilter.credential_filtered_uri(f.uri)}"
      ...
      def download_gem(spec, download_cache_path, previous_spec = nil)
        uri = spec.remote.uri
        CustomBundler.ui.confirm("Fetching #{version_message(spec, previous_spec)}")
        gem_remote_fetcher = remote_fetchers.fetch(spec.remote).gem_remote_fetcher
        CustomBundler.rubygems.download_gem(spec, uri, download_cache_path, gem_remote_fetcher)
      end
      ...
      def download_cache_path(spec)
        return unless CustomBundler.feature_flag.global_gem_cache?
        return unless remote = spec.remote
        return unless cache_slug = remote.cache_slug

        CustomBundler.user_cache.join("gems", cache_slug)
      end
