...
module CustomBundler
  class RubyGemsGemInstaller < Gem::Installer
    def check_executable_overwrite(filename)
      # CustomBundler needs to install gems regardless of binstub overwriting
    end
    ...
    if CustomBundler.rubygems.provides?("< 3.5")
    ...
    if CustomBundler.rubygems.provides?("< 3.5.15")
    ...
    def spec
      if CustomBundler.rubygems.provides?("< 3.3.12") # RubyGems implementation rescues and re-raises errors before 3.3.12 and we don't want that
        @package.spec
      else
        super
      end
    ...
    def prepare_extension_build(extension_dir)
      ...
      require "shellwords" unless CustomBundler.rubygems.provides?(">= 3.2.25")
    end
