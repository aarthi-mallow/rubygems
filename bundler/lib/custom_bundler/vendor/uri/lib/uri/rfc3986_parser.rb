...
module CustomBundler::URI
  class RFC3986_Parser # :nodoc:
    # CustomBundler::URI defined in RFC3986
    HOST = %r[
      (?<IP-literal>\[(?:
          (?<IPv6address>
            (?:\h{1,4}:){6}
    ...
    def split(uri) #:nodoc:
      begin
        ...
        raise InvalidURIError, "bad CustomBundler::URI(is not CustomBundler::URI?): #{uri.inspect}"
      end
      uri.ascii_only? or
        raise InvalidURIError, "CustomBundler::URI must be ascii only #{uri.dump}"
