...
module CustomBundler
  class CLI::Config < Thor
    ...
    desc "base NAME [VALUE]", "The CustomBundler 1 config interface", hide: true
    scope_options
    ...
    class Base
      ...
      def run
        ...
        if options[:delete]
          if !explicit_scope? || scope != "global"
            CustomBundler.settings.set_local(name, nil)
          end
          if !explicit_scope? || scope != "local"
            CustomBundler.settings.set_global(name, nil)
          end
          return
        end
        ...
        if value.nil?
          ...
          if options[:parseable]
            if value = CustomBundler.settings[name]
              CustomBundler.ui.info("#{name}=#{value}")
            end
            return
          end
        ...
        CustomBundler.ui.info(message) if message
        CustomBundler.settings.send("set_#{scope}", name, new_value)
      end
        ...
        if @options[:parseable]
          thor.with_padding do
            CustomBundler.settings.all.each do |setting|
              val = CustomBundler.settings[setting]
              CustomBundler.ui.info "#{setting}=#{val}"
            end
          end
        else
          CustomBundler.ui.confirm "Settings are listed in order of priority. The top value will be used.\n"
          CustomBundler.settings.all.each do |setting|
            CustomBundler.ui.confirm setting
            show_pretty_values_for(setting)
            CustomBundler.ui.confirm ""
          end
        end
      end

      def confirm(name)
        CustomBundler.ui.confirm "Settings for `#{name}` in order of priority. The top value will be used"
      ...
      def message
        locations = CustomBundler.settings.locations(name)
        if @options[:parseable]
