  ...
  class CLI::Inject
    ...
    def run
      ...
      gems.each_slice(4) do |gem_name, gem_version, gem_group, gem_source|
        ...
        deps << CustomBundler::Dependency.new(gem_name, gem_version, "group" => gem_group, "source" => gem_source)
      end

      added = Injector.inject(deps, options)

      if added.any?
        CustomBundler.ui.confirm "Added to Gemfile:"
        CustomBundler.ui.confirm(added.map do |d|
          name = "'#{d.name}'"
        ...
        CustomBundler.ui.confirm "All gems were already present in the Gemfile"
      end
    end
