...
module CustomBundler
  class Settings
    ...
    def set_command_option(key, value)
      if !is_remembered(key) || CustomBundler.feature_flag.forget_cli_options?
        temporary(key => value)
        value
      else
        set_local(key, value)
    ...
    # for legacy reasons, in CustomBundler 2, we do not respect :disable_shared_gems
    def path
    ...
    Path = Struct.new(:explicit_path, :system_path) do
      def path
        path = base_path
        path = File.join(path, CustomBundler.ruby_scope) unless use_system_gems?
        path
      end
      ...
      def use_system_gems?
        ...
        !CustomBundler.feature_flag.default_install_uses_path?
      end

      def base_path
        path = explicit_path
        path ||= ".bundle" unless use_system_gems?
        path ||= CustomBundler.rubygems.gem_dir
        path
      end

      def base_path_relative_to_pwd
        base_path = Pathname.new(self.base_path)
        expanded_base_path = base_path.expand_path(CustomBundler.root)
      ...
      def validate!
        ...
        path = CustomBundler.settings.pretty_values_for(:path)
        path.unshift(nil, "path:") unless path.empty?
        system_path = CustomBundler.settings.pretty_values_for("path.system")
        system_path.unshift(nil, "path.system:") unless system_path.empty?
        disable_shared_gems = CustomBundler.settings.pretty_values_for(:disable_shared_gems)
    ...
    def global_config_file
      ...
      elsif CustomBundler.rubygems.user_home && !CustomBundler.rubygems.user_home.empty?
        Pathname.new(CustomBundler.rubygems.user_home).join(".bundle/config")
      end
    end
    ...
    def load_config(config_file)
      ...
      SharedHelpers.filesystem_access(config_file, :read) do |file|
        ...
        serializer_class.load(file.read).inject({}) do |config, (k, v)|
          ...
          unless k.start_with?("#")
            if k.include?("-")
              CustomBundler.ui.warn "Your #{file} config includes `#{k}`, which contains the dash character (`-`).\n" \
