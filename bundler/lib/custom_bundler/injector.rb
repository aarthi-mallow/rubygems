...
module CustomBundler
  class Injector
    ...
    def self.inject(new_deps, options = {})
      ...
      injector.inject(CustomBundler.default_gemfile, CustomBundler.default_lockfile)
    end

    def self.remove(gems, options = {})
      injector = new(gems, options)
      injector.remove(CustomBundler.default_gemfile, CustomBundler.default_lockfile)
    end
    ...
    def inject(gemfile_path, lockfile_path)
      CustomBundler.definition.ensure_equivalent_gemfile_and_lockfile(true)
      ...
      CustomBundler.settings.temporary(deployment: false, frozen: false) do
        ...
        # invalidate the cached CustomBundler.definition
        CustomBundler.reset_paths!
    ...
    def remove(gemfile_path, lockfile_path)
      ...
      CustomBundler.definition.gemfiles.each do |path|
        deps = remove_deps(path)

        show_warning("No gems were removed from the gemfile.") if deps.empty?

        deps.each {|dep| CustomBundler.ui.confirm "#{SharedHelpers.pretty_dependency(dep)} was removed." }
      end
    ...
    def remove_deps(gemfile_path)
      ...
      CustomBundler.ui.info "Removing gems from #{gemfile_path}"
    ...
    def cross_check_for_errors(gemfile_path, original_deps, removed_deps, initial_gemfile)
      ...
      unless extra_removed_gems.empty?
        ...
        raise InvalidOption, "Gems could not be removed. #{extra_removed_gems.join(", ")} would also have been removed. CustomBundler cannot continue."
      end
    ...
    def show_warning(message)
      CustomBundler.ui.info CustomBundler.ui.add_color(message, :yellow)
    end
