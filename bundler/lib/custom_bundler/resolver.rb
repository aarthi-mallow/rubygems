...
module CustomBundler
  ...
  class Resolver
    ...
    def start
      ...
      CustomBundler.ui.info "Resolving dependencies...", true
    ...
    def setup_solver
      ...
      logger = CustomBundler::UI::Shell.new
    ...
    def solve_versions(root:, logger:)
      ...
      if names_to_relax.any?
        if names_to_unlock.any?
          CustomBundler.ui.debug "Found conflicts with locked dependencies. Will retry with #{names_to_unlock.join(", ")} unlocked...", true

          @base.unlock_names(names_to_unlock)
        end

        if names_to_allow_prereleases_for.any?
          CustomBundler.ui.debug "Found conflicts with dependencies with prereleases. Will retrying considering prereleases for #{names_to_allow_prereleases_for.join(", ")}...", true
        ...
        CustomBundler.ui.debug "Retrying resolution...", true
        retry
      end
    ...
    def no_versions_incompatibility_for(package, unsatisfied_term)
      ...
      if name == "bundler" && bundler_pinned_to_current_version?
        custom_explanation = "the current CustomBundler version (#{CustomBundler::VERSION}) does not satisfy #{constraint}"
    ...
    def name_for_explicit_dependency_source
      CustomBundler.default_gemfile.basename.to_s
    ...
    def raise_not_found!(package)
      ...
      cache_message = begin
                          " or in gems cached in #{CustomBundler.settings.app_cache_path}" if CustomBundler.app_cache.exist?
    ...
    def bundler_not_found_message(conflict_dependencies)
      ...
      if candidate_specs.any?
        ...
        "Your bundle requires a different version of CustomBundler than the one you're running.\n" \
        "Install the necessary version with `gem install bundler:#{target_version}` and rerun bundler using `#{new_command}`\n"
      else
        "Your bundle requires a different version of CustomBundler than the one you're running, and that version could not be found.\n"
      end
    end
  end
end
